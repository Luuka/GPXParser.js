...
gpxParser.prototype.export = function(){
    var exportData = {};

    exportData['waypoints'] = JSON.stringify(this.waypoints);

    var serializer = new XMLSerializer();
    exportData['xmlSource'] = serializer.serializeToString(this.xmlSource);

    exportData['jsonSource'] = JSON.stringify(this.jsonSource);
    exportData['trackpoints'] = JSON.stringify(this.trackpoints);
    exportData['routepoints'] = JSON.stringify(this.routepoints);

    exportData['distance'] = JSON.stringify(this.distance);
    exportData['cumulDistance'] = JSON.stringify(this.cumulDistance);

    exportData['elevation'] = JSON.stringify(this.elevation);
    exportData['heightDifference'] = JSON.stringify(this.heightDifference);

    return JSON.stringify(exportData);
}

gpxParser.prototype.import = function(gpxObject){

  var gpx = JSON.parse(gpxObject);

  this.waypoints = gpx['waypoints'];

  var domParser = new DOMParser();
  var doc = domParser.parseFromString(gpx.xmlSource, 'text/xml');
  this.xmlSource = doc;

  this.jsonSource = gpx['jsonSource'];
  this.trackpoints = gpx['trackpoints'];
  this.routepoints = gpx['routepoints'];

  this.distance = gpx['distance'];
  this.cumulDistance = gpx['cumulDistance'];

  this.elevation = gpx['elevation'];
  this.heightDifference = gpx['heightDifference'];
